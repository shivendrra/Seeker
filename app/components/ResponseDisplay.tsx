...
// Inline markdown parser for a single line
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (match[1]) {
      ...
      elements.push(
        <code
          key={match.index}
          className="bg-gray-100 dark:bg-zinc-800 text-indigo-600 dark:text-indigo-300 text-sm rounded px-1.5 py-0.5 font-mono"
        >
          {match[6]}
        </code>
      );
      ...
      elements.push(
        <sup
          key={match.index}
          className="bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 font-semibold px-1 py-0.5 rounded text-xs ml-1 cursor-pointer"
          title={match[11]}
        >
          {match[11]}
        </sup>
...
const SimpleMarkdown: React.FC<{ text: string }> = ({ text }) => {
  ...
  const flushList = () => {
    if (listItems.length > 0) {
      elements.push(
        <ul key={`ul-${elements.length}`} className="list-disc list-inside ml-6 my-2 space-y-1 text-base leading-relaxed">
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-zinc-700 text-base">
              ...
              <tr>
                {tableHeaders.map((header, i) => (
                  <th
                    key={i}
                    scope="col"
                    className="px-6 py-3 text-left text-sm font-semibold text-gray-700 dark:text-zinc-200 uppercase tracking-wider"
                  >
                    {header.trim()}
                  </th>
                ))}
            ...
            <tbody className="bg-white dark:bg-zinc-800 divide-y divide-gray-200 dark:divide-zinc-700">
              {tableRows.map((row, i) => (
                <tr key={i}>
                  {row.map((cell, j) => (
                    <td key={j} className="px-6 py-3 text-gray-800 dark:text-zinc-300 text-base">
      ...
      elements.push(
        <div key={`code-${elements.length}`} className="bg-zinc-900 text-white rounded-lg my-4 text-sm overflow-hidden">
          ...
          <pre className="p-4 overflow-x-auto text-sm leading-relaxed">
            <code className={`language-${codeBlockLang}`}>{codeBlockLines.join('\n')}</code>
          </pre>
        </div>
      );
      codeBlockLines = [];
  ...
  lines.forEach((line) => {
    ...
    if (headingMatch) {
      ...
      elements.push(
        <Tag
          key={line}
          className={`${level === 1
            ? 'text-2xl font-extrabold mt-8 mb-4'
            : level === 2
              ? 'text-xl font-bold mt-6 mb-3 border-b dark:border-zinc-700 pb-1'
              : level === 3
                ? 'text-lg font-semibold mt-4 mb-2'
                : 'text-base font-semibold mt-3 mb-1'
            } text-gray-900 dark:text-zinc-100 poppins`}
        >
    ...
    if (trimmed.startsWith('> ')) {
      elements.push(
        <blockquote
          key={line}
          className="border-l-4 poppins border-gray-300 dark:border-zinc-600 pl-4 italic text-gray-600 dark:text-zinc-400 my-2 text-base leading-relaxed"
        >
          <MarkdownLine text={trimmed.replace(/^>\s/, '')} />
        </blockquote>
      );
      return;
    }
    ...
    if (trimmed !== '') {
      elements.push(
        <p key={line} className="text-base leading-relaxed text-gray-800 dark:text-zinc-300">
...
const WelcomeScreen: React.FC = () => (
  <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-zinc-400">
    <div className="flex items-center justify-center w-20 h-20 mb-5 rounded-full bg-indigo-50 dark:bg-indigo-900/40">
      <LogoIcon className="w-12 h-12 text-indigo-500 dark:text-indigo-400" />
    </div>
    <h2 className="text-2xl font-bold text-gray-800 dark:text-zinc-100 leading-snug">
      Start Your Research
    </h2>
    <p className="max-w-md mt-2 text-base leading-relaxed">
...
const ResponseDisplay: React.FC<{ messages: Message[] }> = ({ messages }) => {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 poppins">
