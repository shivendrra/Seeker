...
const GoogleIcon: React.FC<{ iconName: string; className?: string }> = ({ iconName, className = '' }) => {
  ...
  useLayoutEffect(() => {
    ...
    const computed = Math.floor(size * 0.92);
  ...
  return (
    <span
      ref={containerRef}
      className={`inline-flex items-center justify-center overflow-hidden align-middle leading-none ${className}`}
      style={{ lineHeight: 1, display: 'inline-flex' }}
      aria-hidden="true"
    >
      <span
        className="material-symbols-outlined"
        style={{ fontVariationSettings: "'FILL' 0, 'wght' 400, 'GRAD' 0, 'opsz' 24", fontSize, lineHeight: 1 }}
      >
        {iconName}
      </span>
    </span>
...
export const GoogleLogoIcon: React.FC<{ className?: string }> = ({ className = '' }) => {
  ...
  useLayoutEffect(() => {
    ...
    if (!el) return;
    const rect = el.getBoundingClientRect();
    const s = Math.max(0, Math.min(rect.width, rect.height));
    setSize(s);
  }, [className]);

  return (
    <span ref={ref} className={`inline-flex items-center justify-center overflow-hidden ${className}`} style={{ lineHeight: 1 }}>
      <svg
        width={size ?? '100%'}
        height={size ?? '100%'}
        viewBox="0 0 48 48"
